...
/**
 * Transformer: honeywellbt Dynamic Media / Scene7 image handling.
 *
 * The source uses Scene7 IS/Image URLs (e.g.
 * https://honeywell.scene7.com/is/image/honeywell/...). 27 such URLs were
 * detected in migration-work/metadata.json `.images.mapping`.
 *
 * This transformer rewrites every DM/Scene7 <img> into an anchor that
 * round-trips through markdown intact:
 *   - unlinked <img>            -> <a href="DM-URL">alt</a>
 *   - linked <img>              -> <a href="/page" title="DM-URL">alt</a>
 *   - mixed-content anchors     -> skipped (no single-anchor markdown form)
 *
 * A companion auto-block in scripts/scripts.js (installed by the
 * site-migration orchestrator) rebuilds these anchors into responsive
 * <picture> elements at render time. See references/dm-scene7-transformer.md.
 *
 * Runs in afterTransform ONLY: block parsers run between the hooks and extract
 * <img> references into block cells; rewriting imgs to anchors in
 * beforeTransform would leave parsers with no img and emit empty cells.
 *
 * Canonical helpers below are kept byte-identical with dm-scene7-helpers.js.
 */
